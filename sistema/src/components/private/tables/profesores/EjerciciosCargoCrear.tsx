...
export default function EjerciciosCargoCrear (): JSX.Element {
  ...
  const getCursos = async (): Promise<void> => {
    try {
      const { data } = await axios.get(`${Global.url}/cursosDelProfesor/${auth.id ?? ''}`, {
        headers: {
  ...
  const saveExamen = async (
    ...
    try {
      ...
      if (status === 201) {
        ...
        navigate(`/admin/ejercicios/cargo/${auth.id}`)
      }
      if (status === 400) {
  ...
  const { handleSubmit, handleChange, errors, values, touched, handleBlur } =
    useFormik({
      initialValues: {
        titulo: '',
        descripcion: '',
        fecha_fin: formatearFechaParaInputDate(new Date().toISOString()),
        fecha_inicio: formatearFechaParaInputDate(new Date().toISOString()),
        tiempo_limite: '',
        puntaje_maxima: '',
        activo: true
      },
        ...
        : (
          ...
          >
            ...
            <div className="flex gap-2 w-full justify-end">
              ...
              <Link
                to={`/admin/ejercicios/cargo/${auth.id}`}
